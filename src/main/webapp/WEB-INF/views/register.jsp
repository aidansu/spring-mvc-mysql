<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <meta name="author" content="">
    <title>注册</title>
</head>

<body>

<div class="container">

    <form class="form-signin" id="userForm" action="" onsubmit="javascript:return false;">
        <h2 class="form-signin-heading">注册</h2>
        <label for="inputUsername" class="sr-only">用户名:</label>
        <input name="username" type="text" id="inputUsername" class="form-control" placeholder="用户名"><br>
        <label for="inputTelephone" class="sr-only">手机号:</label>
        <input name="telephone" type="text" id="inputTelephone" class="form-control" placeholder="手机号"><br>
        <label for="inputPassword" class="sr-only">密 码:</label>
        <input name="password" type="password" id="inputPassword" class="form-control" placeholder="密码" ><br>
        <label for="inputPassword2" class="sr-only">确认密码:</label>
        <input name="password2" type="password" id="inputPassword2" class="form-control" placeholder="确认密码密码" ><br>
        <button id="registerBtn" class="btn btn-lg btn-primary btn-block" type="submit">注 册</button>
    </form>

</div> <!-- /container -->

<br>
<a href="${pageContext.request.contextPath}/">返回主页</a>

<script src="${pageContext.request.contextPath}/js/jquery-1.11.3.min.js" type="text/javascript" ></script>
<script type="text/javascript">
$(document).ready(function(){
    $("#registerBtn").click(function(){
        var formValue = $("#userForm").serialize();
        $.ajax({
            type: "POST",
            cache: false,
            url:"${pageContext.request.contextPath}/registering",
            data:formValue,
            success: function(msg){
                if( "200" == msg.errcode )  {
                    window.location.href = "${pageContext.request.contextPath}/login";
                }else{
                    alert(msg.errmsg);
                }
            }
        });
    });
});
</script>

</body>
</html>
